...
signal interstitial_loaded(placement: String)
signal interstitial_failed(placement: String, message: String)
...
const PLACEMENT_RUN_END_5TH := "run_end_5th"
...
func initialize() -> void:
	...
	if Engine.has_singleton(PLUGIN_NAME):
		...
		_connect_plugin_signal("interstitial_loaded", _on_plugin_interstitial_loaded)
		_connect_plugin_signal("interstitial_failed_to_load", _on_plugin_interstitial_failed_to_load)
		_connect_plugin_signal("interstitial_closed", _on_plugin_interstitial_closed)
		_connect_plugin_signal("interstitial_failed_to_show", _on_plugin_interstitial_failed_to_show)
		_plugin.preloadAllRewarded()
		_plugin.preloadAllInterstitials()
...
func is_interstitial_ready(placement: String) -> bool:
	if _plugin == null:
		return false
	return bool(_plugin.isInterstitialReady(placement))


func show_interstitial(placement: String) -> bool:
	if _plugin == null:
		interstitial_failed.emit(placement, "AdMob Android 플러그인이 현재 실행 환경에 없습니다.")
		return false
	if not bool(_plugin.isInterstitialReady(placement)):
		_plugin.loadInterstitial(placement)
		interstitial_failed.emit(placement, "전면 광고를 불러오는 중입니다.")
		return false
	_plugin.showInterstitial(placement)
	return true


func preload_interstitial(placement: String) -> void:
	if _plugin != null:
		_plugin.loadInterstitial(placement)
...
func _on_plugin_interstitial_loaded(placement: String) -> void:
	interstitial_loaded.emit(placement)


func _on_plugin_interstitial_failed_to_load(placement: String, message: String) -> void:
	interstitial_failed.emit(placement, message)


func _on_plugin_interstitial_closed(placement: String) -> void:
	interstitial_closed.emit(placement)


func _on_plugin_interstitial_failed_to_show(placement: String, message: String) -> void:
	interstitial_failed.emit(placement, message)
