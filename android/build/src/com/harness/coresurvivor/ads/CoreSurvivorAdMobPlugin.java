...
public class CoreSurvivorAdMobPlugin extends GodotPlugin {
    ...
    private final Map<String, String> productionInterstitialUnits = new HashMap<>();
    ...
    private final Set<String> loadingInterstitialPlacements = new HashSet<>();
    ...
    public CoreSurvivorAdMobPlugin(Godot godot) {
        ...
        productionInterstitialUnits.put("run_end_5th", "ca-app-pub-4402708884038037/4642399306");
    }
    ...
    public Set<SignalInfo> getPluginSignals() {
        ...
        signals.add(new SignalInfo("interstitial_loaded", String.class));
        signals.add(new SignalInfo("interstitial_failed_to_load", String.class, String.class));
        signals.add(new SignalInfo("interstitial_closed", String.class));
        signals.add(new SignalInfo("interstitial_failed_to_show", String.class, String.class));
        return signals;
    }
    ...
    @UsedByGodot
    public void preloadAllInterstitials() {
        for (String placement : productionInterstitialUnits.keySet()) {
    ...
    @UsedByGodot
    public boolean isInterstitialReady(String placement) {
        return interstitialAds.containsKey(placement);
    }
    ...
    @UsedByGodot
    public void loadInterstitial(final String placement) {
        if (!productionInterstitialUnits.containsKey(placement)) {
            emitSignal("interstitial_failed_to_load", placement, "Unknown placement: " + placement);
            return;
        }
        if (activity == null) {
            emitSignal("interstitial_failed_to_load", placement, "Activity is not ready.");
            return;
        }
        if (loadingInterstitialPlacements.contains(placement) || interstitialAds.containsKey(placement)) {
            return;
        }
        runOnUiThread(() -> {
            loadingInterstitialPlacements.add(placement);
            InterstitialAd.load(
                    activity,
                    resolveInterstitialAdUnitId(placement),
                    new AdRequest.Builder().build(),
                    new InterstitialAdLoadCallback() {
                        @Override
                        public void onAdLoaded(@NonNull InterstitialAd interstitialAd) {
                            loadingInterstitialPlacements.remove(placement);
                            interstitialAds.put(placement, interstitialAd);
                            emitSignal("interstitial_loaded", placement);
                        }
                        ...
                        public void onAdFailedToLoad(@NonNull LoadAdError loadAdError) {
                            ...
                            String message = loadAdError.getMessage();
                            Log.w(TAG, "Interstitial ad failed to load: " + placement + " / " + message);
                            emitSignal("interstitial_failed_to_load", placement, message);
                        }
                    }
            );
        });
    ...
    public void showInterstitial(final String placement) {
        if (activity == null) {
            emitSignal("interstitial_failed_to_show", placement, "Activity is not ready.");
            return;
        }
        runOnUiThread(() -> {
            InterstitialAd ad = interstitialAds.remove(placement);
            if (ad == null) {
                emitSignal("interstitial_failed_to_show", placement, "Interstitial ad is not ready.");
                loadInterstitial(placement);
                return;
            }
            ad.setFullScreenContentCallback(new FullScreenContentCallback() {
                @Override
                public void onAdDismissedFullScreenContent() {
                    emitSignal("interstitial_closed", placement);
                    loadInterstitial(placement);
                }
                ...
                public void onAdFailedToShowFullScreenContent(@NonNull AdError adError) {
                    String message = adError.getMessage();
                    Log.w(TAG, "Interstitial ad failed to show: " + placement + " / " + message);
                    emitSignal("interstitial_failed_to_show", placement, message);
                    loadInterstitial(placement);
                }

                @Override
                public void onAdShowedFullScreenContent() {
                    Log.d(TAG, "Interstitial ad showed: " + placement);
                }
            });
            ad.show(activity);
        });
    ...
    private String resolveInterstitialAdUnitId(String placement) {
        if (isDebuggable()) {
            return TEST_INTERSTITIAL_ID;
        }
        return productionInterstitialUnits.get(placement);
    }
